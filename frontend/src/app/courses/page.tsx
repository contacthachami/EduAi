...
function courseTime(course: CourseInfo) {
...
function courseGroupKey(course: CourseInfo) {
  return [
...
function getRecentDisplayCourses(courses: CourseInfo[]): DisplayCourse[] {
  const groups = new Map<string, DisplayCourse>();
  const sorted = [...courses].sort((a, b) => courseTime(b) - courseTime(a));
  ...
  for (const course of sorted) {
    ...
    if (existing) {
      ...
      groups.set(key, { ...course, duplicateCount: 1 });
    }
  }

  return Array.from(groups.values()).sort(
    (a, b) => courseTime(b) - courseTime(a),
  );
}
...
export default function CoursesPage() {
  ...
  const recentCourses = useMemo(
    () => getRecentDisplayCourses(courses),
    [courses],
  );
  ...
  const loadCourses = async () => {
    ...
    try {
      ...
      setError(
        "API backend indisponible. Lance .\\scripts\\dev.ps1 ou demarre FastAPI sur le port 8000.",
      );
    } finally {
      setLoading(false);
    }
        ...
        {error && (
          ...
          >
            ...
            <button
              type="button"
              onClick={loadCourses}
              className="ml-auto rounded-card px-2 py-1 text-xs font-semibold underline-offset-2 hover:underline"
            >
              Reessayer
            </button>
          </div>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink-muted">
                        ...
                        {course.duplicateCount > 1 && (
                          <span className="inline-flex items-center rounded-card bg-bg-secondary px-2 py-0.5 text-[11px] font-medium text-ink-muted">
                            {course.duplicateCount} versions
                          </span>
                        )}
                      </div>
                    </div>
